"""
Surgical fine-tuning for Candidate 7 starting from Candidate 6 best checkpoint.
Goal:
1. Clean multi-color generalization: 67.84% -> >= 70.0%
2. Near-duplicate rate: 6.36% -> <= 5.0%
3. Preserve direct colors (>= 98%), RU (100%), EN (>= 98%), and real holdout distance (<= 0.142).
"""
...
def calibrated_duplicate_loss(
    ...
    min_dist: float = 0.045,
    device: torch.device = None,
) -> torch.Tensor:
    """Penalizes active slots whose predicted OKLab distance is < min_dist (0.045)."""
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", default="ml/palettebrain/checkpoints/candidate-6-best.pt")
    ...
    parser.add_argument("--candidate-id", default="candidate-7")
    parser.add_argument("--epochs", type=int, default=6)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--learning-rate", type=float, default=5e-6)
    parser.add_argument("--weight-decay", type=float, default=1e-4)
    parser.add_argument("--dup-weight", type=float, default=0.30)
    parser.add_argument("--mc-weight", type=float, default=1.95)
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs, eta_min=args.learning_rate * 0.3)
    ...
    print(f"\nStarting {args.candidate_id} targeted calibration for {args.epochs} epochs...\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            dup_loss = calibrated_duplicate_loss(raw_pred, count_mask, min_dist=0.045, device=device)
        ...
        score = (mc_val_acc * 3.5) - avg_val_loss if eligible else -999.0
